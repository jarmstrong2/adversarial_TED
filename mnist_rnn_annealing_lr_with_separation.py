...
class RNN_MNIST_model(object):
	def __init__(self, config, is_training = True, model_type="FULL"):
		...
		if model_type == "GEN" or model_type == "FULL":
			...
			init_image = tf.zeros([batch_size,(28/splits[split_opt])*(28/splits[split_opt])])
			...
			g_w = tf.get_variable("RNN_g_input_target_w", [((28/splits[split_opt])*(28/splits[split_opt]))+10, hidden_size_RNN_g])
			...
			output_max = tf.tile(output_max, [1,1,(28/splits[split_opt])*(28/splits[split_opt])])
		...
		if model_type == "DISC" or model_type == "FULL":
			if model_type == "DISC":
				self.image_input = tf.placeholder(tf.float32, [batch_size, splits[split_opt]**2, (28/splits[split_opt])*(28/splits[split_opt])])
			else:
			...
			final_output = tf.slice(output, [0,(splits[split_opt]**2)-1,0], [batch_size, 1, hidden_size_RNN_d])
...
def getinput(batch_x):
	...
	segment_size = 28/splits[split_opt]

	for i in range(splits[split_opt]):
		for j in range(splits[split_opt]):
			split = full_im[:,(i * segment_size):((i+1) * segment_size), 
			(j * segment_size):((j+1) * segment_size)]
			split = np.reshape(split, (configobj().batch_size/2, segment_size*segment_size))
...
def getMovingAvg(trainables):
	...
	return rollingsum/float(rollingcount)
	...
	with tf.Graph().as_default(), tf.Session() as session:
		...
		for i in range(configobj().iterations):
			if ((i+1) % 1000 == 0):
				...
				print("Avg GEN: {}, Avg DISC: {}".format(getMovingAvg(trainable_g), getMovingAvg(trainable_d)))
				...
				plt.savefig('classification_sep_7_2.png')
			...
			if ((i+1) % 10000 == 0):
				save_path = saver.save(session, "model_sep_7_2.ckpt")
